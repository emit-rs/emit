...
The `span_guard!` macro returns a tuple of [`SpanGuard`](https://docs.rs/emit/1.20.0/emit/span/struct.SpanGuard.html) for completing the span, and [`Frame`](https://docs.rs/emit/1.20.0/emit/frame/struct.Frame.html) for activating the span's ambient trace and span ids for correlation.

The syntax accepted by the `span_guard!` macro is the same as the [`#[span]`](https://docs.rs/emit/1.20.0/emit/attr.span.html) attribute.
...
`emit` also defines a [`Span`](https://docs.rs/emit/1.20.0/emit/span/struct.Span.html) type, which are a kind of [`Event`](https://docs.rs/emit/1.20.0/emit/struct.Event.html) specialized for spans in a distributed trace. You can construct a `Span` event manually:
