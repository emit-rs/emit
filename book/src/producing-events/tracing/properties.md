...
With a [`SpanGuard`](https://docs.rs/emit/2.22.1/emit/span/struct.SpanGuard.html), you can attach additional properties collection to the span:
...
- **Shared:** Added to the ambient context and present on all child events. Properties you add to the [`#[span]`](https://docs.rs/emit/2.22.1/emit/attr.span.html) template are shared.
- **Private:** Added to the [`SpanGuard`](https://docs.rs/emit/2.22.1/emit/span/struct.SpanGuard.html) and only present on the span event itself. Properties you add through the `evt_props` [control parameter](../../reference/control-parameters.md), and subsequently through the `SpanGuard` are private.
