...
You can use the [`Delta`](https://docs.rs/emit/1.16.1/emit/metric/struct.Delta.html) type to implement metric sources that track deltas instead of cumulative values. The `Delta` type tracks the range the value covers, automatically updating it when sampled.
...
See [Reporting sources](./reporting-sources.md) for details on how to sample a [`Source`](https://docs.rs/emit/1.16.1/emit/metric/source/trait.Source.html).
