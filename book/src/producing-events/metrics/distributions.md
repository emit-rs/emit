...
`emit` also defines a raw [`midpoint`](https://docs.rs/emit/2.22.1/emit/metric/exp/fn.midpoint.html) function, returning a [`Point`](https://docs.rs/emit/2.22.1/emit/metric/exp/struct.Point.html) that can be stored in a `BTreeMap` or `HashMap` to build your own exponential histogram storage.
