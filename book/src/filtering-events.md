...
`emit` supports client-side filtering using a [`Filter`](https://docs.rs/emit/1.18.0/emit/trait.Filter.html).

## Setup

Filters are configured through the [`setup`](https://docs.rs/emit/1.18.0/emit/setup/fn.setup.html) function at the start of your application by calling [`emit_when`](https://docs.rs/emit/1.18.0/emit/setup/struct.Setup.html#method.emit_when):
...
Filters can be combined with [`and_when`](https://docs.rs/emit/1.18.0/emit/trait.Filter.html#method.and_when) and [`or_when`](https://docs.rs/emit/1.18.0/emit/trait.Filter.html#method.or_when):
...
You can also wrap an emitter in [`emit_to`](https://docs.rs/emit/1.18.0/emit/setup/struct.Setup.html#method.emit_to) in a filter:
...
When you use `emit_when`, the filter may be by-passed using the `when` [control parameter](./reference/control-parameters.md) on the [`emit!`](https://docs.rs/emit/1.18.0/emit/macro.emit.html) or [`#[span]`](https://docs.rs/emit/1.18.0/emit/attr.span.html) macros to emit an event even if the filter wouldn't match it. However, the filter specified by `emit_when` doesn't allow you to filter differently if you specify multiple emitters.
...
You can use the [`level::min_filter`](https://docs.rs/emit/1.18.0/emit/level/fn.min_filter.html) function to create a filter that matches events based on their level:
...
See [the crate docs](https://docs.rs/emit/1.18.0/emit/level/struct.MinLevelFilter.html) for more details.

## Filtering by module

You can use the [`level::min_by_path_filter`](https://docs.rs/emit/1.18.0/emit/level/fn.min_by_path_filter.html) function to create a filter that matches events based on their module path and level:
