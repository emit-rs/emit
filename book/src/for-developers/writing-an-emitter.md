...
You can write a simple emitter using [`emitter::from_fn`](https://docs.rs/emit/1.20.0/emit/emitter/fn.from_fn.html), but advanced cases need to implement the [`Emitter`](https://docs.rs/emit/1.20.0/emit/trait.Emitter.html) trait.
...
If your emitter is complex enough to need its own diagnostics, you can add the `implicit_internal_rt` feature of `emit` and use it when calling [`emit!`](https://docs.rs/emit/1.20.0/emit/macro.emit.html) or [`#[span]`](https://docs.rs/emit/1.20.0/emit/attr.span.html):
...
Emitters should minimize their impact in calling code by offloading expensive processing to a background thread. You can use [`emit_batcher`](https://docs.rs/emit_batcher/1.20.0/emit_batcher/index.html) to implement a batching, retrying, asynchronous emitter.
