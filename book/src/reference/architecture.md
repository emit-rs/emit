...
- [`emit`](https://docs.rs/emit/1.13.1/emit/index.html): The main library that re-exports `emit_core` and `emit_macros`. This is the one your applications depend on.
- [`emit_core`](https://docs.rs/emit_core/1.13.1/emit_core/index.html): Just the fundamental APIs. It includes the `shared()` and `internal()` runtimes. The goal of this library is to remain stable, even if macro syntax evolves over time.
- [`emit_macros`](https://docs.rs/emit_macros/1.13.1/emit_macros/index.html): `emit!`, `#[span]`, and other procedural macros.
...
- [`emit_term`](https://docs.rs/emit_term/1.13.1/emit_term/index.html): Writes to the console. See [Emitting to the console](../emitting-events/console.md) for details.
- [`emit_file`](https://docs.rs/emit_file/1.13.1/emit_file/index.html): Writes to rolling files. See [Emitting to rolling files](../emitting-events/rolling-files.md) for details.
- [`emit_otlp`](https://docs.rs/emit_otlp/1.13.1/emit_otlp/index.html): Writes OpenTelemetry's wire protocol. See [Emitting via OTLP](../emitting-events/otlp.md) for details.

You can also write your own emitters by implementing the [`Emitter`](https://docs.rs/emit/1.13.1/emit/trait.Emitter.html) trait. See [Writing an Emitter](../for-developers/writing-an-emitter.md) for details.

## Events

[`Event`](https://docs.rs/emit/1.13.1/emit/struct.Event.html)s are the central data type in `emit` that all others hang off. They look like this:
...
- A [`Path`](https://docs.rs/emit/1.13.1/emit/struct.Path.html) for the component that generated them.
- A [`Template`](https://docs.rs/emit/1.13.1/emit/struct.Template.html) for their human-readable description. Templates can also make good low-cardinality identifiers for a specific shape of event.
- An [`Extent`](https://docs.rs/emit/1.13.1/emit/struct.Extent.html) for the time the event is relevant. The extent itself may be a single [`Timestamp`](https://docs.rs/emit/1.13.1/emit/struct.Timestamp.html) for a point in time, or a pair of timestamps representing an active time range.
- [`Props`](https://docs.rs/emit/1.13.1/emit/trait.Props.html) for structured key-value pairs attached to the event. These can be lazily interpolated into the template.

See [Event data model](./events.md) for more details.

## Runtimes

In `emit`, a diagnostic pipeline is an instance of a [`Runtime`](https://docs.rs/emit/1.13.1/emit/runtime/struct.Runtime.html). Each runtime is an isolated set of components that help construct and emit diagnostic events in your applications. It looks like this:
...
A [`Runtime`](https://docs.rs/emit/1.13.1/emit/runtime/struct.Runtime.html) includes:

- [`Emitter`](https://docs.rs/emit/1.13.1/emit/trait.Emitter.html): Responsible for sending events to some outside observer.
- [`Filter`](https://docs.rs/emit/1.13.1/emit/trait.Filter.html): Responsible for determining whether an event should be emitted or not.
- [`Ctxt`](https://docs.rs/emit/1.13.1/emit/trait.Ctxt.html): Responsible for storing ambient context that's appended to events as they're constructed.
- [`Clock`](https://docs.rs/emit/1.13.1/emit/trait.Clock.html): Responsible for assigning timestamps to events and running timers.
- [`Rng`](https://docs.rs/emit/1.13.1/emit/trait.Rng.html): Responsible for generating unique identifiers like trace and span ids.

An [`AmbientSlot`](https://docs.rs/emit/1.13.1/emit/runtime/struct.AmbientSlot.html) is a container for a `Runtime` that manages global initialization. `emit` includes two built-in ambient slots:

- [`shared()`](https://docs.rs/emit/1.13.1/emit/runtime/fn.shared_slot.html): The runtime used by default when not otherwise specified.
- [`internal()`](https://docs.rs/emit/1.13.1/emit/runtime/fn.internal_slot.html): The runtime used by other runtimes for self diagnostics.

You can also define your own `AmbientSlot`s or use `Runtime`s directly.

## Event construction and emission

When the [`emit!`](https://docs.rs/emit/1.13.1/emit/macro.emit.html) macro is called, an event is constructed using features of the runtime before being emitted through it. This is how it works:
...
When the [`#[span]`](https://docs.rs/emit/1.13.1/emit/attr.span.html) macro is called, the annotated function is instrumented using features of the runtime before a span representing its execution is emitted through it. This is how it works:
