...
All diagnostics in `emit` are represented as an [`Event`](https://docs.rs/emit/2.22.0/emit/struct.Event.html).

Each event is the combination of:

- `mdl` ([`Path`](https://docs.rs/emit/2.22.0/emit/struct.Path.html)): The path of the component that generated the event.
- `tpl` ([`Template`](https://docs.rs/emit/2.22.0/emit/struct.Template.html)): A lazily-rendered, user-facing description of the event.
- `extent` ([`Extent`](https://docs.rs/emit/2.22.0/emit/struct.Extent.html)): The point in time that the event occurred at, or the span of time for which it was active.
- `props` ([`Props`](https://docs.rs/emit/2.22.0/emit/trait.Props.html)): A set of key-value pairs associated with the event.

Here's an example of an event created using the [`emit!`](https://docs.rs/emit/2.22.0/emit/macro.emit.html) macro:
...
The core event data model doesn't encode any specific diagnostic paradigm. It doesn't even include log levels. `emit` uses well-known properties to support extensions to its data model. A well-known property is a reserved name and set of allowed values that consumers of diagnostic data can use to treat an event as something more specific. See the [`well_known`](https://docs.rs/emit/2.22.0/emit/well_known/index.html) module for a complete list of well-known properties.

The two main extensions to the event data model are [tracing](../producing-events/tracing/data-model.md), and [metrics](../producing-events/metrics/data-model.md). Tracing events are represented by the [`Span`](https://docs.rs/emit/2.22.0/emit/span/struct.Span.html) type, and metric events by the [`Metric`](https://docs.rs/emit/2.22.0/emit/metric/struct.Metric.html) type. Each of these types implements the [`ToEvent`](https://docs.rs/emit/2.22.0/emit/event/trait.ToEvent.html) trait, and so can be treated as a regular [`Event`](https://docs.rs/emit/2.22.0/emit/struct.Event.html) within a runtime for filtering and emission.

You can also define your own extensions by setting the [`evt_kind`](https://docs.rs/emit/2.22.0/emit/well_known/constant.KEY_EVT_KIND.html) well-known property to a value of your choice. Consumers that aren't specially aware of your extension will treat those events as regular ones.

## Value data model

The [`Value`](https://docs.rs/emit/2.22.0/emit/struct.Value.html) type is `emit`'s representation of an anonymous structured value based on the [`value_bag`](https://docs.rs/value_bag) library. `Value` is a concrete type rather than a trait to make working with them in [`Props`](https://docs.rs/emit/2.22.0/emit/trait.Props.html) easier. Internally, a value holds a direct reference or embedded primitive value for:
...
The time-oriented part of an event is its [`Extent`](https://docs.rs/emit/2.22.0/emit/struct.Extent.html). Internally, an extent stores [`Timestamp`](https://docs.rs/emit/2.22.0/emit/struct.Timestamp.html)s. An extent can either store one or a pair of timestamps.
...
Events don't have to be constructed using macros. You can use the [`Event::new`](https://docs.rs/emit/2.22.0/emit/struct.Event.html#method.new) constructor manually:
