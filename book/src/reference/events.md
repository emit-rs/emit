...
All diagnostics in `emit` are represented as an [`Event`](https://docs.rs/emit/0.11.5/emit/struct.Event.html).

Each event is the combination of:

- `mdl` ([`Path`](https://docs.rs/emit/0.11.5/emit/struct.Path.html)): The path of the component that generated the event.
- `tpl` ([`Template`](https://docs.rs/emit/0.11.5/emit/struct.Template.html)): A lazily-rendered, user-facing description of the event.
- `extent` ([`Extent`](https://docs.rs/emit/0.11.5/emit/struct.Extent.html)): The point in time that the event occurred at, or the span of time for which it was active.
- `props` ([`Props`](https://docs.rs/emit/0.11.5/emit/trait.Props.html)): A set of key-value pairs associated with the event.

Here's an example of an event created using the [`emit!`](https://docs.rs/emit/0.11.5/emit/macro.emit.html) macro:
...
The time-oriented part of an event is its [`Extent`](https://docs.rs/emit/0.11.5/emit/struct.Extent.html). Internally, an extent stores [`Timestamp`](https://docs.rs/emit/0.11.5/emit/struct.Timestamp.html)s. An extent can either store one or a pair of timestamps.
...
Events don't have to be constructed using macros. You can use the [`Event::new`](https://docs.rs/emit/0.11.5/emit/struct.Event.html#method.new) constructor manually:
