...
The [`emit!`](https://docs.rs/emit/2.22.0/emit/macro.emit.html) and [`#[span]`](https://docs.rs/emit/2.22.0/emit/attr.span.html) macros use the same syntax.
...
You can control how properties are rendered within templates by implementing the [`template::Write`](https://docs.rs/emit/2.22.0/emit/template/trait.Write.html) trait. `emit_term` uses this for example to render different property types in different colors.
