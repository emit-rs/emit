# Summary

[About](./about.md)

- [Getting started](./getting-started.md)
- [Producing events](./producing-events.md)
    - [Logging](./producing-events/logging.md)
        - [Data model](./producing-events/logging/data-model.md)
        - [Key types](./producing-events/logging/types.md)
        - [Log event creation](./producing-events/logging/creation.md)
        - [Attaching properties](./producing-events/logging/properties.md)
        - [Attaching errors](./producing-events/logging/errors.md)
        - [Log levels](./producing-events/logging/levels.md)
        - [Ambient context](./producing-events/logging/context.md)
        - [Performance](./producing-events/logging/performance.md)
    - [Tracing](./producing-events/tracing.md)
        - [Data model](./producing-events/tracing/data-model.md)
        - [Key types](./producing-events/tracing/types.md)
        - [Span creation](./producing-events/tracing/creation.md)
        - [Manual span creation](./producing-events/tracing/manual-span-creation.md)
        - [Manual span completion](./producing-events/tracing/manual-span-completion.md)
        - [Attaching properties](./producing-events/tracing/properties.md)
        - [Tracing fallible functions](./producing-events/tracing/fallible-functions.md)
        - [Propagating context across threads](./producing-events/tracing/propagating-across-threads.md)
        - [Propagating context across services](./producing-events/tracing/propagating-across-services.md)
        - [Sampling and filtering](./producing-events/tracing/sampling.md)
        - [Span links](./producing-events/tracing/links.md)
        - [Wide events](./producing-events/tracing/wide-events.md)
        - [Limitations](./producing-events/tracing/limitations.md)
        - [Performance](./producing-events/tracing/performance.md)
    - [Metrics](./producing-events/metrics.md)
        - [Data model](./producing-events/metrics/data-model.md)
        - [Key types](./producing-events/metrics/types.md)
        - [Metric sample creation](./producing-events/metrics/creation.md)
        - [Attaching properties](./producing-events/metrics/properties.md)
        - [Cumulative metrics](./producing-events/metrics/cumulative-metrics.md)
        - [Delta metrics](./producing-events/metrics/delta-metrics.md)
        - [Distributions](./producing-events/metrics/distributions.md)
        - [Reporting from sources](./producing-events/metrics/reporting-sources.md)
        - [Limitations](./producing-events/metrics/limitations.md)
        - [Performance](./producing-events/metrics/performance.md)
    - [Quick debugging](./producing-events/quick-debugging.md)
    - [In tests](./producing-events/testing.md)
- [Filtering events](./filtering-events.md)
- [Working with events](./working-with-events.md)
- [Emitting events](./emitting-events.md)
    - [Console](./emitting-events/console.md)
    - [Rolling files](./emitting-events/rolling-files.md)
    - [Via OTLP](./emitting-events/otlp.md)
- [Advanced apps](./advanced-apps.md)
    - [WebAssembly](./advanced-apps/web-assembly.md)
    - [Embedded applications](./advanced-apps/embedded.md)
    - [Integrating with OpenTelemetry](./advanced-apps/integrating-with-open-telemetry.md)
    - [Setup outside of `main`](./advanced-apps/non-main-setup.md)
- [For developers](./for-developers.md)
    - [Writing an emitter](./for-developers/writing-an-emitter.md)
- [Troubleshooting](./troubleshooting.md)
- [Reference](./reference.md)
    - [Architecture](./reference/architecture.md)
    - [Events](./reference/events.md)
    - [Template syntax and rendering](./reference/templates.md)
    - [Property capturing](./reference/property-capturing.md)
    - [Property attributes](./reference/property-attributes.md)
    - [Control parameters](./reference/control-parameters.md)
    - [Acknowledgements](./reference/acknowledgements.md)
    - [Design notes](./reference/design.md)
