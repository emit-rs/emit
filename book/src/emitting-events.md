...
Diagnostic events produced by `emit` are sent to an [`Emitter`](https://docs.rs/emit/1.11.0/emit/trait.Emitter.html). `emit` provides a few implementations in external libraries you can use in your applications:

- [`emit_term`](https://docs.rs/emit_term/1.11.0/emit_term/index.html) for [emitting to the console](./emitting-events/console.md).
- [`emit_file`](https://docs.rs/emit_file/1.11.0/emit_file/index.html) for [emitting to rolling files](./emitting-events/rolling-files.md).
- [`emit_otlp`](https://docs.rs/emit_otlp/1.11.0/emit_otlp/index.html) for [emitting via OTLP](./emitting-events/otlp.md).

## Setup

Emitters are configured through the [`setup`](https://docs.rs/emit/1.11.0/emit/setup/fn.setup.html) function at the start of your application by calling [`emit_to`](https://docs.rs/emit/1.11.0/emit/setup/struct.Setup.html#method.emit_to):
...
Once initialized, any subsequent calls to [`init`](https://docs.rs/emit/1.11.0/emit/setup/struct.Setup.html#method.init) will panic.

`emit_to` will replace any previously set emitter during the same setup. You can set multiple emitters by calling [`and_emit_to`](https://docs.rs/emit/1.11.0/emit/setup/struct.Setup.html#method.and_emit_to):
...
You can map an emitter to a new value by calling [`map_emitter`](https://docs.rs/emit/1.11.0/emit/setup/struct.Setup.html#method.map_emitter):
...
You can also treat a [`Filter`](https://docs.rs/emit/1.11.0/emit/trait.Filter.html) as a wrapping directly:
...
Events may be processed asynchronously, so to ensure they're fully flushed before your `main` returns, you can call [`blocking_flush`](https://docs.rs/emit/1.11.0/emit/setup/struct.Init.html#method.blocking_flush) at the end of your `main` function:
...
Instead of `blocking_flush`, you can call [`flush_on_drop`](https://docs.rs/emit/1.11.0/emit/setup/struct.Init.html#method.flush_on_drop):
