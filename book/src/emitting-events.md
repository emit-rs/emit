...
Diagnostic events produced by `emit` are sent to an [`Emitter`](https://docs.rs/emit/0.11.0-alpha.19/emit/trait.Emitter.html). `emit` provides a few implementations in external libraries you can use in your applications:

- [`emit_term`](https://docs.rs/emit_term/0.11.0-alpha.19/emit_term/index.html) for [emitting to the console](./emitting-events/console.md).
- [`emit_file`](https://docs.rs/emit_file/0.11.0-alpha.19/emit_file/index.html) for [emitting to rolling files](./emitting-events/rolling-files.md).
- [`emit_otlp`](https://docs.rs/emit_otlp/0.11.0-alpha.19/emit_otlp/index.html) for [emitting via OTLP](./emitting-events/otlp.md).

## Setup

Emitters are configured through the [`setup`](https://docs.rs/emit/0.11.0-alpha.19/emit/setup/fn.setup.html) function at the start of your application by calling [`emit_to`](https://docs.rs/emit/0.11.0-alpha.19/emit/setup/struct.Setup.html#method.emit_to):
...
Once initialized, any subsequent calls to [`init`](https://docs.rs/emit/0.11.0-alpha.19/emit/setup/struct.Setup.html#method.init) will panic.

`emit_to` will replace any previously set emitter during the same setup. You can set multiple emitters by calling [`and_emit_to`](https://docs.rs/emit/0.11.0-alpha.19/emit/setup/struct.Setup.html#method.and_emit_to):
...
You can map an emitter to a new value by calling [`map_emitter`](https://docs.rs/emit/0.11.0-alpha.19/emit/setup/struct.Setup.html#method.map_emitter):
...
Emitters can be treated like middleware using a [`Wrapping`](https://docs.rs/emit/0.11.0-alpha.19/emit/emitter/wrapping/trait.Wrapping.html) by calling [`Emitter::wrap_emitter`](https://docs.rs/emit/0.11.0-alpha.19/emit/trait.Emitter.html#method.wrap_emitter). Wrappings are functions over an [`Emitter`](https://docs.rs/emit/0.11.0-alpha.19/emit/trait.Emitter.html) and [`Event`](https://docs.rs/emit/0.11.0-alpha.19/emit/event/struct.Event.html) that may transform the event before emitting it, or discard it altogether.
...
You can also treat a [`Filter`](https://docs.rs/emit/0.11.0-alpha.19/emit/trait.Filter.html) as a wrapping directly:
