...
| `evt_props`    | `impl emit::Props`              | A set of properties to attach to the span guard (see the `guard` parameter) that will appear on the emitted span event, but not pushed to the ambient context. The type that will appear on the `SpanGuard` is an anonymous wrapper that dereferences to the concrete type of `evt_props`. |
| `name`         | `str`                           | The name of the span, carried in the well-known `span_name` property. If unspecified the template literal is used.                                                                                                                                                                         |
| `kind`         | `str` or `emit::span::SpanKind` | The kind of the span, carried in the well-known `span_kind` property. If unspecified the kind is left empty.                                                                                                                                                                               |
...
Some control parameters have side effects in the annotated function that can be observed through their order of execution. The order of evaluation is:
