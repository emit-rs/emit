...
fn assert_emitter(
    ...
    emit::emit!(rt, extent: emit::clock().now()..emit::clock().now(), "A span {span_uuid}", evt_kind: "span", span_name: "emit_otlp_test", trace_id: emit::TraceId::random(emit::rng()), span_id: emit::SpanId::random(emit::rng()));
